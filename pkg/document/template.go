...
var (
    ...
    downloadedTemplates = make(map[string]string)
...
func getRemoteTemplate(templateFile string) (string, error) {
    downloadMutex.Lock()
    defer downloadMutex.Unlock()

    // Check if already downloaded
    if tempPath, exists := downloadedTemplates[templateFile]; exists {
        log.Debugf("Using cached template for %s at %s", templateFile, tempPath)
        return tempPath, nil
    }

    // Create temp file
    tmpFile, err := os.CreateTemp("", "helm-docs-template-*.gotmpl")
    if err != nil {
    ...
    // Download the file
    log.Debugf("Downloading template from %s", templateFile)
    resp, err := http.Get(templateFile)
    if err != nil {
        return "", fmt.Errorf("failed to download template %s: %w", templateFile, err)
    }
    defer resp.Body.Close()

    if resp.StatusCode != http.StatusOK {
        return "", fmt.Errorf("failed to download template, got status code %d", resp.StatusCode)
    }
    ...
    // Cache the result
    downloadedTemplates[templateFile] = tmpPath
...
func getDocumentationTemplate(chartDirectory string, chartSearchRoot string, templateFiles []string) ([]string, error) {
	...
	for _, templateFile := range templateFiles {
		...
		} else if strings.HasPrefix(templateFile, "https://") || strings.HasPrefix(templateFile, "http://") {
			downloadPath, err := getRemoteTemplate(templateFile)
			if err != nil {
					log.Warnf("Failed to download template %s: %s", templateFile, err)
			}
			fullTemplatePath = downloadPath
		} else {
